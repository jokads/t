...
class MT5Client:
    ...
    async def start(self):
        ...
        logger.info(
            f"Starting MT5Client (server mode: {self.config.host}:{self.config.port})"
        )
        self.running = True
    ...
    async def execute_order(
        self, signal: SignalCreate, timeout: float = 5.0
    ) -> Dict[str, Any]:
        """
    ...
    async def _connect(self) -> bool:
        ...
        try:
            logger.info(
                f"Waiting for MT5 EA connection on {self.config.host}:{self.config.port}"
            )
            ...
            try:
                if not self.connected:
                    ...
                    if self.reconnect_attempts > 0:
                        ...
                        logger.info(
                            f"Reconnecting in {delay:.1f}s (attempt {self.reconnect_attempts + 1})"
                        )
            ...
            try:
                if self.connected:
                    ...
                    if now - self.last_heartbeat_time > self.config.heartbeat_timeout:
                        ...
                        # Send ping
                        heartbeat = Heartbeat(type="heartbeat.ping", sender="python")
