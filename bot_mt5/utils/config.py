...
class AIConfig(BaseModel):
    ...
    pool_size: int = Field(
        default=2, ge=1, le=8, description="Number of worker processes per model"
    )
    timeout_quick: float = Field(
        default=8.0, gt=0, description="Timeout for quick AI calls (seconds)"
    )
    timeout_deep: float = Field(
        default=30.0, gt=0, description="Timeout for deep analysis AI calls (seconds)"
    )
    max_retries: int = Field(
        default=3, ge=0, description="Max retries for failed AI calls"
    )
    ...
    def from_env(cls) -> AIConfig:
        ...
        return cls(
            model_paths=(
                os.getenv("AI_MODEL_PATHS", "").split(":")
                if os.getenv("AI_MODEL_PATHS")
                else None
            ),
...
class MT5Config(BaseModel):
    ...
    protocol: Literal["socket", "dll"] = Field(
        default="socket", description="Communication protocol"
    )
    reconnect_max_attempts: int = Field(
        default=10, ge=1, description="Max reconnection attempts"
    )
    ...
    reconnect_backoff_max: float = Field(
        default=60.0, gt=0, description="Max backoff delay (seconds)"
    )
    heartbeat_interval: float = Field(
        default=30.0, gt=0, description="Heartbeat interval (seconds)"
    )
    heartbeat_timeout: float = Field(
        default=60.0, gt=0, description="Heartbeat timeout (seconds)"
    )
    exec_timeout: float = Field(
        default=5.0, gt=0, description="Order execution timeout (seconds)"
    )
...
class RateLimitConfig(BaseModel):
    ...
    orders_per_minute: int = Field(
        default=60, ge=1, description="Max orders per minute per account"
    )
...
class PerformanceConfig(BaseModel):
    ...
    use_orjson: bool = Field(
        default=True, description="Use orjson for JSON serialization"
    )
